...
void tampilData(data*mahasiswa){
    ...
    for(int i=0;i<size;i++){
        ...
        printf("%d\t%-15lld\t%-25s\t%d\t%d\t%d\t%d\t%f\t%d\t%d\t%s\n",i+1,mahasiswa->nrp[i],mahasiswa->mahasiswa[i],mahasiswa->nilai[(i)*4+0],mahasiswa->nilai[(i)*4+1],mahasiswa->nilai[(i)*4+2],mahasiswa->nilai[(i)*4+3],rata,mahasiswa->util[(i)*2],mahasiswa->util[(i)*2+1], rata>85?"A":rata>80?"A-":rata>75?"AB":rata>70?"B+":rata>65?"B":rata>60?"BC":rata>55?"C":rata>40?"D":"E");
